...
runSmoke('sync-pull-dump', async (ctx) => {
  ...
  const server = {
     ...
     * 수단(통장) 하나로 좁힌 목록.
     *
     * 이 통장에서 오간 돈 전부다 -- 나간 것도, 수입도, 이체로 받은 것도 든다. 서버와
     * 사본이 각자 조건을 손으로 만드는 자리라(entrySearchConditions ↔ searchFilter)
     * 갈리면 같은 통장을 눌러도 온라인·오프라인 목록이 달라진다.
     */
    methodEntries: (
     ...
     * 카드 하나로 좁힌 목록.
     *
     * 쓴 것과 갚은 것이 함께 들어야 한다 (대금 결제의 카드 다리는 양수다). 서버와
     * 사본이 각자 조건을 만드는 자리라 갈리면 같은 카드를 눌러도 온라인·오프라인
     * 목록이 달라진다.
     */
    cardMethodEntries: (
      await entries.getEntries(uid, { paymentCardIds: credit.id, limit: 200 }, pid)
    ).data.map((row) => row.id),
