...
export class CardsController {
  ...
  @ApiOperation({
    summary: '카드 삭제 (거래내역이 있으면 거절). hide=true 면 숨기기 (되돌리려면 PATCH isActive=true)',
  })
  delete(
    @Request() req: AuthenticatedRequest,
    @Param('id') id: string,
    ...
    return hide === 'true'
      ? this.cardsService.deactivateCard(id, req.user.id)
      : this.cardsService.deleteCard(id, req.user.id);
  }
}
