...
export class ConfigService {
  ...
  /**
   * 갱신 토큰의 수명. 이 값이 곧 **오프라인의 실제 상한**이다.
   *
   * 앱은 오프라인에서도 계속 적을 수 있지만, 이 수명이 지나면 다음에 앱을 열 때
   * 로그인 화면이 뜬다. 적어 둔 것은 잃지 않는다 -- 기기 사본과 큐는 그대로 두고
   * (설계 문서 D10), 같은 계정으로 다시 들어오면 밀린 것이 올라간다. 로그인 화면이
   * 그 사실을 건수와 함께 알려 준다.
   *
   * **늘리기 전에 알아야 할 것.** 이 토큰은 서버가 기억하지 않는 상태 없는 토큰이라
   * (아래 logout 참고) 한번 나가면 만료 전에는 끊을 수 없다. 30일로 늘리면 유출된
   * 토큰이 30일 동안 유효하고, 그것을 끊을 방법은 지금 없다. 늘리려면 먼저 끊을
   * 방법(발급 목록이나 tokenVersion)을 두는 것이 순서다.
   */
