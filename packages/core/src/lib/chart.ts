...
/**
 * 원형차트 조각 색. 분류별 상세가 쓴다.
 *
 * 위의 CHART_CATEGORY_COLORS 와 따로 둔다. 그쪽은 선·막대처럼 갈래가 적고 이름표가
 * 붙는 그림의 색이라 여덟뿐인데, 분류는 열두 갈래가 예사다. 조각마다 이름과 금액을
 * 함께 적으므로 색만으로 갈라 볼 일은 없다.
 *
 * 열둘을 넘으면 앞에서부터 돌려 쓴다. 웹과 앱이 같은 조각을 같은 색으로 그려야 해서
 * core 에 둔다.
 */
