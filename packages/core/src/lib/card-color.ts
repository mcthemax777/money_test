...
export interface CardPalette {
  ...
  labelKey: MessageKey;
  /** 앞면 그라데이션 (웹의 tailwind 클래스) */
  face: string;
  /**
   * 같은 그라데이션의 색 정지점.
   *
   * 앱에는 CSS 그라데이션이 없어 클래스를 읽을 수 없다. 위 face 와 같은 색을 같은
   * 차례로 적어 두고, 앱은 이 값으로 직접 그린다. 둘을 따로 고치면 두 화면의 카드
...
export const CARD_PALETTE: Record<CardColor, CardPalette> = {
  ...
  copper: {
    ...
    faceColors: ['#7c3f1d', '#c1743c', '#9a5327'],
    ...ON_DARK,
    positive: { text: 'text-lime-200', bar: 'bg-lime-200' },
    negative: { text: 'text-red-300', bar: 'bg-red-300' },
  },
  ...
  gold: {
    ...
    faceColors: ['#ca8a04', '#fbbf24', '#eab308'],
    ...ON_LIGHT,
    ink: 'text-amber-950',
  ...
  silver: {
    ...
    faceColors: ['#aeb6c0', '#f1f3f6', '#98a1ad'],
    ...ON_LIGHT,
    positive: { text: 'text-emerald-700', bar: 'bg-emerald-700' },
    negative: { text: 'text-red-700', bar: 'bg-red-700' },
  },
