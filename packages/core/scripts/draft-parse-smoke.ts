...
{
  /*
   * 7 을 따옴표와 슬래시로 읽은 자리. 사진에서 자주 온다.
   *
   * "08월27일" 이 "08월2'/일" 로 왔다. 되살리지 않으면 읽다 만 머리로 걸러져 앞 날짜를
   * 이어 쓰고, 8월 27일 거래가 8월 28일로 담긴다(2026-09-09에 실제로 겪었다).
   *
   * 숫자 자리를 아예 잃은 머리("08월2'일")는 되살릴 수 없다. 그때는 앞서 본 날짜를
   * 이어 쓴다 -- 그대로 읽어 8월 2일로 담는 것보다 이웃한 날짜가 가깝다.
   */
  const text = [
    '08월28일',
    '스마트로 대표비인증 25,000원',
  ...
  eq('따옴표+슬래시는 숫자 7 이다', localKey(drafts[1]?.occurredAt ?? null), '2026-08-27 12:00');
  eq('숫자를 잃은 머리는 앞 날짜를 이어 쓴다', localKey(drafts[2]?.occurredAt ?? null), '2026-08-27 12:00');
}

{
  // 같은 자리는 금액과 시각에도 온다. 고치지 않으면 후보가 되지 못하거나 시각을 잃는다.
  const text = ['09월04일', "관악구보건소 1'/,000원", "1'/:4'/ 일시불"].join('\n');
  const drafts = parseCaptureText(text, { now: NOW });
  eq('금액의 7 도 되살린다', drafts[0]?.amount, '17000');
  eq('시각의 7 도 되살린다', localKey(drafts[0]?.occurredAt ?? null), '2026-09-04 17:47');
}

{
  /*
   * 기기 OCR 이 자릿점을 점으로, "원"을 숫자로 읽은 자리 (2026-09-09).
   *
   *   "1,650원" -> "l.65021"   (첫 자리는 글자 l, 자릿점은 점, 원은 21)
   *   "2,800원" -> "2.8001"
   *
   * 고치지 않으면 금액이 없는 줄로 읽혀 그 거래가 통째로 빠진다 -- 같은 사진에서
   * 웹이 21건, 앱이 19건을 담은 까닭이 이 두 줄이었다.
   */
  const text = [
    '08월31일',
    '지하철 1 건 l.65021',
  ...
  const drafts = parseCaptureText(text, { now: NOW });
  eq('첫 자리의 글자를 숫자 1 로 본다', drafts[0]?.amount, '1650');
  eq('원을 숫자로 읽은 뒤에도 자릿점을 찾는다', drafts[1]?.amount, '2800');
  eq('두 줄 다 후보가 된다', drafts.length, 2);
  eq('금액 뒤에 남은 글자는 금액에 넣지 않는다', drafts[1]?.merchant, '카페 타샤');
}
...

{
   ...
   *
   * 목록이 날짜순이니 "앞 날짜보다 새것이면 버린다"가 그럴듯해 보이지만, 스크롤 캡처는
   * 같은 구간을 두 번 담아 사진 안에서 순서가 되돌아간다. 그 검사를 걸었더니 제대로
   * 되살린 "09월04일"("09204")을 버려 그 아래 4건이 하루 어긋났다(2026-09-09).
   */
  const text = ['08월31일', '카페 타샤 2,800원', '09803', '어향 27,000'].join('\n');
  const drafts = parseCaptureText(text, { now: NOW });
  eq('앞 날짜보다 새것이어도 쓴다', localKey(drafts[1]?.occurredAt ?? null), '2026-09-03 12:00');
}
