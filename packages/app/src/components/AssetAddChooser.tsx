...
import { CreditCard, Landmark, Plus, UserPlus, Wallet, type LucideIcon } from 'lucide-react-native';
...
/**
 * 자산 추가 단추의 그림. 지갑에 작은 더하기 배지를 단다 (웹과 같은 모양).
 *
...
export function AssetAddIcon() {
  return (
    <View>
      ...
      <View className="absolute -bottom-1 -right-1.5 h-3.5 w-3.5 items-center justify-center rounded-full bg-white">
        <Plus size={10} strokeWidth={3.5} color="#2563eb" />
      </View>
    </View>
  );
}
 ...
 * 있어야 만들어진다. 웹의 같은 이름 컴포넌트와 같은 모양이다.
 */
      ...
      <View className="gap-2">
        {options.map(({ kind, label, Icon, blocked }) => (
          <Pressable
            ...
            className={`flex-row items-center gap-3 rounded-lg border border-gray-200 px-4 py-3 active:bg-gray-100 ${
              blocked ? 'opacity-50' : ''
            }`}
          >
            ...
            <View className="shrink">
              ...
              {blocked ? <Text className="text-xs text-gray-500">{blocked}</Text> : null}
            </View>
          </Pressable>
        ))}
      </View>
