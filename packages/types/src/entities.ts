...
export interface EntryListItem {
  ...
  amount: string;
  /**
   * 결제 자리에서 깎인 금액. 차감이 없으면 null.
   *
   * 카드 포인트 사용, 자동할인, 그리고 **취소**가 여기 든다. 셋은 전표에서 같은 모양이다
   * -- 정가는 그대로인데 계좌에서 빠지는 돈만 적다.
   *
   * 위 `amount` 는 이미 이 값을 뺀 뒤라, 편집 화면이 정가 칸을 되돌리려면 둘을 더한다.
   * 전액을 깎았으면 `amount` 가 0 이고 그 거래는 0원으로 남는다.
   */
  discountAmount: string | null;
  /**
   * 이 거래를 카드 실적에 세는가. 카드로 낸 거래에만 뜻이 있다.
   *
   * 실적과 청구액은 다른 값이다 -- 꺼 두어도 갚을 대금은 그대로다. 기본값은 갈래마다
   * 다르고(지출 포함, 카드 수입 제외), 편집 화면이 이 값을 그대로 되돌려 준다.
   */
